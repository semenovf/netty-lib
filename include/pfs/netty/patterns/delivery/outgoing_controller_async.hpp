...
//      2025.07.01 `outgoing_controller` renamed to `outgoing_controller_async`.
////////////////////////////////////////////////////////////////////////////////
#pragma once
#include "../../error.hpp"
...
// ATTENTION! Do not use this outgoing messages controller yet
...
class outgoing_controller_async
{
...
public:
    outgoing_controller_async (address_type receiver_addr, std::uint32_t part_size = 16384U // 16 Kb
        ...
        for (auto & x: _items) {
            for (int i = 0; i < RETRANSMIT_LIMIT && !x.again.empty(); i++) {
                ...
                PFS__THROW_UNEXPECTED(mt->check_range(sn)
                    , "Fix delivery::outgoing_controller_async algorithm: serial number is out of bounds");
