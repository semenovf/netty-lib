...
pfs::optional<inet4_addr> inet4_addr::parse (char const * s, std::size_t n)
{
    ...
    return inet4_addr{a, b, c, d};
}

pfs::optional<inet4_addr> inet4_addr::parse (char const * s)
{
    return parse(s, std::char_traits<char>::length(s));
}

pfs::optional<inet4_addr> inet4_addr::parse (std::string const & s)
{
    return parse(s.c_str(), s.size());
}

pfs::optional<inet4_addr> inet4_addr::parse (pfs::string_view s)
{
